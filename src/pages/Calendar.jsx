...
import { format, parseISO, addMonths, subMonths, startOfDay, endOfMonth } from "date-fns";
import {
  Search,
  CalendarDays,
  Briefcase,
...
const totalPages = Math.ceil(eventsForDate.length / cardsPerPage);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-blue-100 px-6 py-8 flex flex-col gap-8">

      {/* Header */}
      <div className="flex flex-col items-center gap-4">
  <h1 className="text-4xl md:text-5xl font-extrabold text-blue-950 flex items-center gap-4">
    <Rocket className="w-10 h-10 text-blue-800" /> Explore Opportunities
  </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-3 bg-white rounded-3xl shadow-2xl p-6 flex flex-col gap-6">
          ...
          {/* <div className="flex gap-6 overflow-x-auto pb-4 pt-2">
            {eventsForDate.length>0 ? eventsForDate.map((event,idx)=>(
                ...
                {event.reminders?.length>0 && (
                  <div className="bg-white rounded-2xl shadow-md p-4 border border-blue-100 flex flex-col gap-3">
                    ...
                    {event.reminders.map((rem,rIdx)=>(
                      <div
                        key={rIdx}
                        className="flex justify-between items-center p-3 rounded-xl transition-all hover:shadow-lg hover:scale-[1.02] cursor-pointer border-l-4 border-transparent hover:border-blue-700 bg-gradient-to-r from-blue-50 to-blue-100"
                        title={rem.label}
                      >
                        <div className="flex items-center gap-4 flex-grow">
                          <span
                            className={`w-4 h-4 rounded-full ${getReminderColor(rem.label)} animate-pulse`}
                          />
                          <span className="text-blue-900 font-medium">{rem.label}</span>
                        </div>
                        <span className="text-sm text-blue-700 font-semibold ml-4">
                          {format(parseISO(rem.date), "dd MMM")}
                        </span>
                      </div>
                    ))}
                  </div>
      ...
      >
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-6 flex flex-col gap-6 border border-blue-100">
          ...
          <div className="mt-4 text-sm">
            {/* <p className="font-medium text-blue-900 mb-2">Legend:</p> */}
            <div className="flex flex-col gap-2">
              {[
                ["bg-blue-800", "Internship (Main Event)"],
                ["bg-blue-500", "Hackathon (Main Event)"],
                ["bg-red-500", "Application Deadline"],
                ["bg-orange-400", "Registration"],
                ["bg-yellow-400", "Prototype Submission"],
                ["bg-purple-500", "Idea Submission"],
                ["bg-green-500", "Interview / Pitch / Demo"],
              ].map(([color, label], i) => (
